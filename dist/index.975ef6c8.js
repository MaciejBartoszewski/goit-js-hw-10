...
(0, _catApi.fetchBreeds)().then((cats)=>renderNewOption(cats)).catch((error)=>{
    (0, _notiflixNotifyAio.Notify).failure(error);
    errorAlert();
});
select.addEventListener("change", (e)=>{
    (0, _catApi.fetchCatByBreed)(e.target.value).then((cat)=>generateImage(cat)).catch((error)=>{
        (0, _notiflixNotifyAio.Notify).failure(error);
        errorAlert();
    });
});
...
function generateImage(cat) {
    ...
    return fetch(`https://api.thecatapi.com/v1/images/${newBreedId}`).then((response)=>{
        if (!response.ok) {
            (0, _notiflixNotifyAio.Notify).failure(response.status);
            errorAlert();
        }
...
},{"./cat-api":"d7YdZ","notiflix/build/notiflix-notify-aio":"eXQLZ"}],"d7YdZ":[function(require,module,exports) {
...
function fetchBreeds() {
    return fetch("https://api.thecatapi.com/v1/breeds", {
        headers: {
            Authoryzation: {
                "x-api-key": "live_41i6SveXjchRpTWZglqfBlRTGUsxB2IfpZfHTW6DSkYdNVCFcelFWp8H2ftmv7Ie"
            }
        }
    }).then((response)=>{
        if (!response.ok) (0, _notiflixNotifyAio.Notify).failure(response.status);
        return response.json();
    });
}
